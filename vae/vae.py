...
class VariationalAutoEncoder:
    ...
    def _build(self):
        ...
        self.mu = k.layers.Dense(self.z_dim, name='mu')(x)
        self.log_var = k.layers.Dense(self.z_dim, name='log_var')(x)

        # encoder_output = k.layers.Dense(self.z_dim, name='encoder_output')(x)

        # self.encoder_z_dist = k.models.Model(encoder_input, (self.mu, self.log_var))

        def sampling(args):
            mu, log_var = args
            epsilon = k.backend.random_normal(shape=k.backend.shape(mu), mean=0., stddev=1.)
            return mu + k.backend.exp(log_var / 2) * epsilon

        encoder_output = k.layers.Lambda(sampling, name='encoder_output')([self.mu, self.log_var])
    ...
    def compile(self, lr, r_loss_factor):
        ...
        def vae_r_loss(y_true, y_pred):
            r_loss = k.backend.mean(k.backend.square(y_true - y_pred), axis=[1, 2, 3])
            return r_loss_factor * r_loss

        def vae_kl_loss(y_true, y_pred):
            return -0.5 * k.backend.sum(1 + self.log_var - k.backend.square(self.mu) - k.backend.exp(self.log_var), axis=1)

        def ver_loss(y_true, y_pred):
            r_loss = vae_r_loss(y_true, y_pred)
            kl_loss = vae_kl_loss(y_true, y_pred)

            return r_loss + kl_loss

        self.model.compile(optimizer=optimizer, loss=ver_loss, metrics=[vae_r_loss, vae_kl_loss])
    ...
    def train(self, x_train, batch_size, epochs, lr_decay=1):
        ...
        monitor = MonitoringCallback(self)
...
ae = VariationalAutoEncoder(input_dim=(28, 28, 1),
...
if MODE == 'TRAIN':
    ...
    ae.compile(lr=LEARNING_RATE, r_loss_factor=R_LOSS_FACTOR)
    ae.train(x_train, batch_size=BATCH_SIZE, epochs=100)
else:
